...
internal abstract class BasePostNetworkCommand : NetworkCommand {
    ...
    override fun sendRequest(body: String?): String? {
        ...
        try {
            ...
            LogUtils.debug("[NetworkCommand] Begin request:\nurl: $url\nbody: $body")
            ...
            LogUtils.debug("[NetworkCommand] Body sent")
            ...
            return response.toString().also {
                LogUtils.debug("[NetworkCommand] Response received\n$it")
            }
        } catch (e: Throwable) {
            LogUtils.debug("[NetworkCommand] Error", e)
            throw Exception(e)
        } finally {
